...
const hostIndex = async (req, res) => {
  ...
  try {
    ...
    */
    ...
    if (catDoc) {
      catName = catDoc.catName;
    }

    // if (dogDoc) {
    //   dogName = dogDoc.dogName;
    // }
  } catch (err) {
...
const searchName = async (req, res) => {
  ...
  */
  ...
  return res.json({ name: doc.name, breed: doc.breed, age: { $inc: { 'doc.age': 1 } } });
};
...
const updateLast = (req, res) => {
  ...
  */
  ...
  updatePromise.then((doc) => {
    if (!doc) {
      return res.status(404).json({ error: 'No cat found to update' });
    }
